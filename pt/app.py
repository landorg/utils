...
def generate_html_page(rank: str | None = None, error: str | None = None, competition_display_name: str | None = None, task_number: str | None = None) -> str:
    ...
    if rank:
        ...
        if context_html: title = f"Rank {html.escape(rank)} - {comp_esc} T{task_esc}"
        else: title = f"Rank: {html.escape(rank)}"
    elif error:
        content = f'<div id="error-display">{html.escape(error)}</div>'; title = "Error"
    else:
        content = '<div id="error-display">No data</div>'; title = "Error"
...
def get_rank_widget():
    ...
    if not pilot_id:
        ...
        return Response(generate_html_page(error=error_msg), mimetype='text/html', status=400)
    ...
    if not active_task_info:
        ...
        return Response(generate_html_page(error=error_msg), mimetype='text/html', status=404)
    ...
    if isinstance(results_data, str) and results_data.startswith("ERROR:"):
        ...
        return Response(generate_html_page(error=error_msg, competition_display_name=competition_display_name, task_number=task_num), mimetype='text/html', status=500)
    ...
    if found_rank:
        logger.info(f"Pilot ID {pilot_id} found with rank {found_rank}. Displaying rank.")
        ...
        logger.warning(f"Pilot ID {pilot_id} not found in active task {comp_slug}/Task {task_num}.")
        error_msg = f"Pilot ID {html.escape(pilot_id)} not found."
        return Response(generate_html_page(error=error_msg, competition_display_name=competition_display_name, task_number=task_num), mimetype='text/html', status=404)
